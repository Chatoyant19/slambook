...
Chessboard::Chessboard(cv::Size board_size, cv::Mat& image)
 : board_size_(board_size)
 , corners_found_(false)
{
    if (image.channels() == 1)
    {
        cv::cvtColor(image, sketch_, CV_GRAY2BGR);
        image.copyTo(mImage);
    }
    else
    {
...
{
    mCornersFound = findChessboardCorners(mImage, board_size_, mCorners,
    ...
    if (mCornersFound)
    {
        ...
        cv::drawChessboardCorners(mSketch, board_size_, mCorners, mCornersFound);
    }
}
